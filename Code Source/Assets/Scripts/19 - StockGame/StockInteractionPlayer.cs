...
public class StockInteractionPlayer : MonoBehaviour
{
    ...
    void Update()
    {
        if (!(Time.timeScale == 0))
        {
                ...
                foreach (Touch touch in Input.touches)
                {
                    ...
                    switch (touch.phase)
                    {
                        ...
                        case TouchPhase.Moved:
                            this.OnTouchDragged(currentTouch.position);
                            //								stwp = Camera.main.ScreenToWorldPoint (touch.position);
                            //								touchPos = new Vector2 (stwp.x, stwp.y);
                            //				
                            //								if (this.TestHit (touchPos)) {
            ...
            else
            {
                ...
                {
                    this.OnTouchDragged(Input.mousePosition);
                }
                else if (Input.GetAxis("Mouse X") == 0)
                {
                    this.OnTouchStayed();
    ...
    public void OnTouchDragged(Vector2 position)
    {
        switch (etat)
        {
            case State.UNTOUCHED:
                break;
            case State.TOUCHED:
                if (idFingerTouched == currentTouch.fingerId || !Input.touchSupported)
                {
                    this.transform.position = Camera.main.ScreenToWorldPoint(new Vector3(
                        position.x, position.y, 0) - cameraOffsetZ);
                    etat = State.TOUCHED;
    ...
    {
        bool xSup = touchPos.x < GetComponent<Collider2D>().bounds.max.x;
        bool xInf = touchPos.x > GetComponent<Collider2D>().bounds.min.x;
        bool ySup = touchPos.y < GetComponent<Collider2D>().bounds.max.y;
        bool yInf = touchPos.y > GetComponent<Collider2D>().bounds.min.y;
